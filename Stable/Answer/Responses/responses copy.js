...
function hoist_response(resp_no)
{
    ...
    if (this_resp == "admit")
    {
        response_long.innerText = `Defendant admits the allegations contained within paragraph ${resp_no}.`;
    }
    else if (this_resp == "deny")
    {
        response_long.innerText = `Defendant denies the allegations contained within paragraph ${resp_no}.`;
    }
    else if (this_resp == "insuff")
    {
        response_long.innerText = `Defendant, after reasonable investigation, lacks sufficient information to admit or deny the allegations contained within paragraph ${resp_no}.`;
    }
    else if (this_resp == "legal")
    {
        response_long.innerText = `The allegations contained within paragraph ${resp_no} call for a legal conclusion not subject to factual admission or denial.`;
    }
    else if (this_resp == "not_subject")
    {
        response_long.innerText = `The allegations contained within paragraph ${resp_no} are not subject to factual admission or denial.`;
    }
